...
export default function DiagnosticsPage({ params }: PageProps) {
    const diagnostics = diagnosticsData[params.slug as keyof typeof diagnosticsData]
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="mb-8">
                            <div className="flex items-center justify-center gap-4 mb-4">
                                ...
                                {diagnostics.price && <span className="text-2xl font-bold text-pink-600">{diagnostics.price}</span> }
                            </div>
                            <h1 className="text-3xl font-bold mb-4 text-balance text-center">{diagnostics.name}</h1>
                            {diagnostics.description && <p className="text-lg italic text-muted-foreground text-balance text-center">{diagnostics.description}</p> }
                        </div>

                        {/* Image */}
                        {diagnostics.image && <div className="relative h-64 md:h-80 rounded-lg overflow-hidden mb-8">
                            <Image
                                src={diagnostics.image || "/placeholder.svg?height=400&width=800&query=medical+iv+diagnostics"}
                                alt={diagnostics.name}
                                fill
                    ...
                    <div className="lg:col-span-1">
                        <Card className="p-6 sticky top-24">
                            ...
                            <BookingButton className={"mt-4"} service={{id: diagnostics.name, title: diagnostics.name}} />
                            <div className="text-center">
                                <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground mt-2">
                                    <Clock className="h-4 w-4" />
                                    <span>{diagnostics.duration}</span>
                                </div>
                            </div>
                        </Card>
                    </div>
